...
for loader_func, dataset_name in zip([get_diabetes_data_and_labels, get_breast_cancer_data_and_labels],
    ...
    # section 3
    x_train, y_train = shuffle_dataset(x_train, y_train)
    perform_simple_algorithm_loo(x_train=x_train, y_train=y_train, linear_params=linear_params, rbf_params=rbf_params,
                                 title=f'simple algorithm loo {dataset_name} dataset', verbose=True)
    perform_rand_algorithm_loo(x_train=x_train, y_train=y_train, linear_params=linear_params, rbf_params=rbf_params,
                                 title=f'rand algorithm loo {dataset_name} dataset', verbose=True)
