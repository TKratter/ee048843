...
from ex1.scripts.bayes_error import estimate_bayes_error
...
save_path = None
synthetic_loader_list = [lambda: create_dataset_by_dimention(2),
                         lambda: create_dataset_by_dimention(50),
                         lambda: create_dataset_by_dimention(100),
                         lambda: create_dataset_by_dimention(200)]
...
for loader_func, dataset_name in zip(loaders_list, dataset_names_list):
    ...
    if 'dim' in dataset_name:
        estimate_bayes_error(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, clf_params=linear_params)
